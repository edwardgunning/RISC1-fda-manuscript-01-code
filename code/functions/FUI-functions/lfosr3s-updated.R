...
lfosr3s_updated <- function(formula, data, family = "gaussian", argvals = NULL, var = FALSE, analytic = TRUE,
  ...
  if(analytic == TRUE){ ## we need smoothing parameter, spline basis and penalty matrix for analytic inference
    ...
    for(r in 1:p){
      fit_smooth <- gam(betaTilde[r,] ~ s(argvals, bs = "cr", k = (nknots + 1)), method = "GCV.Cp")
