...
settings <- expand.grid(bf_covs = c(5, 8, 10, 15),
            mfpc_cutoff = c(0.9, 0.95))
mind <- 1
results_list <- vector(mode = "list", length = nrow(settings))
time_vec <- vector("numeric", length = nrow(settings))
# mind = "model index"
for(mind in seq_len(nrow(settings))) {
  print(paste0("iteration ", mind, " of ", nrow(settings)))
  time_vec[mind] <- system.time(results_list[[mind]] <- fit_multifamm(data = mfamm_data,
                bf_covs = rep(settings[mind, "bf_covs"], 2),
                mfpc_cutoff = settings[mind, "mfpc_cutoff"]
                ))["elapsed"]
  gc()
  }
saveRDS(list(time_vec=time_vec, results_list=results_list, settings = settings),
        file = here::here("outputs/results/mfamm-comparison.rds"))
